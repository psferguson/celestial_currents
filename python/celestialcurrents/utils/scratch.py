...
class SurveyConfig:
    # make init function
    def __init__(self, survey, base_directory):
        if survey.lower() not in [s.lower() for s in SURVEY]:
            logger.error(f"survey must be in {SURVEY}")
            raise ValueError
        else:
            self.survey = survey
...
filename = "~/software/celestial_currents/DECaLS_DR10/DECaLS_DR10_iso_hpxcube_z0.0001_a12.0_gmax23.0_gmin16.fits.gz"

ml = MapLoader(survey='DECaLS_DR10', 
               base_directory='~/software/celestial_currents/DECaLS_DR10/', 
               z=0.0001, 
               age=12.0, 
               mag_max=23.0, 
               mag_min=16)
