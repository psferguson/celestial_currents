...
class MapLoader:
    ...
    def get_filename(self, name_fstring=None):
        if name_fstring is None:
            name_fstring = (
                "{survey}_iso_hpxcube_z{z:.4f}_"
                "a{age:.1f}_gmax{mag_max}_"
                "gmin{mag_min}.fits.gz"
            )

        filename = self.data_directory + name_fstring.format(
            survey=self.survey,
            z=self.z,
            age=self.age,
            mag_max=self.mag_max,
            mag_min=int(self.mag_min),
        )
        return filename
    ...
    def load_hpxcube(filename):
        ...
        with fitsio.FITS(filename) as f:
            if not ("HPXCUBE" in f):
                logger.error(f"HPXCUBE extension not in {filename}")
                raise ValueError
            else:
                hpxcube = f["HPXCUBE"].read()

            if "FRACDET" not in f:
                logger.debug("skipping fracdet...")
                fracdet = None
            else:
                fracdet = f["FRACDET"].read()
                logger.info(
                    f"fracdet read with {np.sum(fracdet > 0.5)} covered pixels"
                )

            if "MODULUS" not in f:
                logger.debug("no modulus array found...")
                modulus = np.array([16.0])
            else:
